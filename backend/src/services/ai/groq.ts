...
export class GroqProvider extends BaseAIProvider {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (!this.apiKey) {
        throw new AIError('Groq API key is required', 'API_KEY_MISSING', 500);
      }
  ...
  private async callGroqAPI(messages: Array<{ role: string; content: string }>): Promise<any> {
