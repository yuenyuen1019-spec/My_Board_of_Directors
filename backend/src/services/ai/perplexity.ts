...
export class PerplexityProvider extends BaseAIProvider {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (!this.apiKey) {
        throw new AIError('Perplexity API key is required', 'API_KEY_MISSING', 500);
      }
    ...
    try {
      ...
      const aiResponse: AIResponse = {
        ...
        metadata: {
          ...
          responseTime,
          confidence: 0.85, // Perplexity 通常有較高的置信度
        },
      };
  ...
  getModelInfo(): ModelInfo {
    return {
      ...
      capabilities: ['text-generation', 'conversation', 'reasoning', 'web-search'],
    };
  }
  ...
  private async testConnection(): Promise<void> {
    ...
    await this.callPerplexityAPI(testMessages);
  }

  private async callPerplexityAPI(messages: Array<{ role: string; content: string }>): Promise<any> {
